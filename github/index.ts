...
import { createOpencodeClient } from "@exa/sdk"
...
try {
  ...
  console.log("exa session", session.id)
  if (shareId) {
...
function createOpencode() {
  ...
  const proc = spawn(`exa`, [`serve`, `--hostname=${host}`, `--port=${port}`])
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/exa|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/exa` or `/oc`")
  }
}
...
async function assertOpencodeConnected() {
  ...
  if (!connected) {
    throw new Error("Failed to connect to exa server")
  }
}
...
async function getAccessToken() {
  ...
  if (isMock()) {
    ...
    const oidcToken = await core.getIDToken("exa-github-action")
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    ...
    if (body.includes("/exa") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to exa...")
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `exa/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/exa-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[exa session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
