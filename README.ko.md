...
# 패키지 매니저
npm i -g exa-ai@latest        # bun/pnpm/yarn 도 가능
scoop install exa             # Windows
choco install exa             # Windows
brew install anomalyco/tap/exa # macOS 및 Linux (권장, 항상 최신)
brew install exa              # macOS 및 Linux (공식 brew formula, 업데이트 빈도 낮음)
sudo pacman -S exa            # Arch Linux (Stable)
paru -S exa-bin               # Arch Linux (Latest from AUR)
mise use -g exa               # 어떤 OS든
nix run nixpkgs#exa           # 또는 github:anomalyco/exa 로 최신 dev 브랜치
```
...
Exa 는 데스크톱 앱으로도 제공됩니다. [releases page](https://github.com/anomalyco/exa/releases) 에서 직접 다운로드하거나 [opencode.ai/download](https://opencode.ai/download) 를 이용하세요.
...
Exa 에는 내장 에이전트 2개가 있으며 `Tab` 키로 전환할 수 있습니다.
...
Exa 설정에 대한 자세한 내용은 [**문서**](https://opencode.ai/docs) 를 참고하세요.

### 기여하기

Exa 에 기여하고 싶다면, Pull Request 를 제출하기 전에 [contributing docs](./CONTRIBUTING.md) 를 읽어주세요.

### Exa 기반으로 만들기

Exa 와 관련된 프로젝트를 진행하면서 이름에 "exa"(예: "exa-dashboard" 또는 "exa-mobile") 를 포함한다면, README 에 해당 프로젝트가 Exa 팀이 만든 것이 아니며 어떤 방식으로도 우리와 제휴되어 있지 않다는 점을 명시해 주세요.

---

**커뮤니티에 참여하기** [Discord](https://discord.gg/exa) | [X.com](https://x.com/exa)
