...
import { LocationServiceMap } from "@exa/core/location-service-map"
import { SessionExecutionLocal } from "@exa/core/session/execution/local"
...
export function createRoutes(password?: string) {
  return makeRoutes(
    password
      ? ServerAuth.Config.configLayer({ username: "exa", password: Option.some(password) })
      : ServerAuth.Config.layer,
  )
}

export function createEmbeddedRoutes() {
  return makeRoutes(ServerAuth.Config.configLayer({ username: "exa", password: Option.none() }))
}
