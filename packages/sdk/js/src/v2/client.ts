...
import { ExaClient } from "./gen/sdk.gen.js"
import { wrapClientError } from "../error-interceptor.js"
export { type Config as ExaClientConfig, ExaClient }
...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-exa-directory", "directory"],
    ["x-exa-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-exa-directory")
  next.headers.delete("x-exa-workspace")
  return next
}

export function createExaClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  if (!config?.fetch) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-exa-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-exa-workspace": config.experimental_workspaceID,
    }
  }
  ...
  client.interceptors.response.use((response) => {
    ...
    if (contentType === "text/html")
      throw new Error("Request is not supported by this version of Exa Server (Server responded with text/html)")
