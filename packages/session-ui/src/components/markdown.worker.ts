...
import { createMarkdownParser } from "@exa/ui/context/marked-parser"
import { ExaTheme } from "@exa/ui/context/marked-theme"
import {
  bundledLanguages,
  createHighlighter,
  getTokenStyleObject,
...
const parser = createMarkdownParser(async (code, language) => {
  ...
  return instance.codeToHtml(code, { lang: name as BundledLanguage, theme: "Exa", tabindex: false })
})
...
async function highlight(request: Extract<MarkdownWorkerRequest, { type: "highlight" }>) {
  try {
    ...
    if (request.complete) {
      const result = instance.codeToTokens(request.text, { lang: language as BundledLanguage, theme: "Exa" })
    ...
    const stream = reset
      ? {
          language,
          source: "",
          tokenizer: new ShikiStreamTokenizer({ highlighter: instance, lang: language, theme: "Exa" }),
        }
      : previous
...
function getHighlighter() {
  return (highlighter ??= createHighlighter({ themes: [ExaTheme], langs: [] }))
}
