...
import type { UpdaterState } from "@exa/app/updater"
...
import "./styles.css"
import { Splash } from "@exa/ui/logo"
import { useTheme } from "@exa/ui/theme/context"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.EXA_CHANNEL === "prod" &&
...
const deepLinkEvent = "exa:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__EXA__ ??= {}
  const pending = window.__EXA__.deepLinks ?? []
  window.__EXA__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `exa.desktop.window.${windowID}.last-active-url`
}
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("exa.global.dat").getItem("language")
