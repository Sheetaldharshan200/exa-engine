...
const TEST_ONBOARDING = process.env.EXA_TEST_ONBOARDING === "1"
const SIDECAR_VERSION = process.env.EXA_SIDECAR_V2 === "1" ? "v2" : "v1"
...
const main = Effect.gen(function* () {
  ...
  process.env.EXA_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.exa.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `exa-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Exa Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("exa://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("exa")
  registerRendererProtocol()
  setDockIcon()
  ...
  const loadingTask = yield* Effect.gen(function* () {
    ...
    const port = yield* Effect.gen(function* () {
      const fromEnv = process.env.EXA_PORT
      if (fromEnv) {
