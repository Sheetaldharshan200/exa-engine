...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "Exa update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Exa", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.exa/bin/exa",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, exaChecks: {} })
})
...
test("ignores stale background Exa checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  expect(controller.getState().exaChecks).toEqual({})
})

test("ignores stale startup Exa checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking Exa", async () => {
  persistedServers = []
  ...
  expect(exa).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().exaChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check Exa in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const exa: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(exa).toEqual(["Debian"])
  expect(Object.keys(controller.getState().exaChecks)).toEqual(["Debian"])
})
