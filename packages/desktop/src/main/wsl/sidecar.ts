...
export async function spawnWslSidecar(
  ...
  const exa = await resolveWslOpencode(distro)
  if (!exa) throw new Error(nativeT("desktop.wsl.error.exaNotInstalled", { distro }))
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export EXA_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export EXA_CLIENT=desktop",
    `export EXA_SERVER_USERNAME=${shellEscape(username)}`,
    `export EXA_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(exa)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
