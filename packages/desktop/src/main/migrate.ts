...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.exa.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `exa.settings.dat` is special: it maps to the `exa.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"exa.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // exa.settings.dat → the electron settings store ("exa.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "exa.global.dat").
  const storeName = filename === "exa.settings.dat" ? "exa.settings" : filename
