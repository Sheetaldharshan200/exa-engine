...
> current private `@exa/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@exa/ai`
...
- Secondary audience: Exa and other durable agent runtimes
...
import { LLM } from "@exa/ai"
import { OpenAI } from "@exa/ai/providers/openai"
...
Exa and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@exa/ai/promise"
import { OpenAI } from "@exa/ai/providers/openai"
...
```ts
import { LLMSchema } from "@exa/ai/schema"
...
```ts
import { Provider, Protocol } from "@exa/ai/provider"
...
@exa/ai/providers/openai
@exa/ai/providers/anthropic
@exa/ai/providers/google
...
```
...
Exa should migrate to `generateTurn` / `streamTurn`, preserving its durable
