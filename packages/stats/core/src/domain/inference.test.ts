...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve exa route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "exa")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "exa")).toBe("openai")
    expect(statProvider("big-pickle", "", "exa")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "exa"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep exa as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "exa"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "exa"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep exa or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "exa"), country: "US" })).toMatchObject([
