...
export const EXA_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const EXA_MODE_KEY = "exa.mode"

export const ExaKeymapProvider = KeymapProvider
export const useExaKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type ExaModeStack = ReturnType<typeof createExaModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, ExaModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createExaModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(EXA_MODE_KEY, EXA_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(EXA_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(EXA_MODE_KEY, stack.at(-1)?.mode ?? EXA_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? EXA_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(EXA_MODE_KEY, undefined)
...
export function useExaModeStack() {
  return getExaModeStack(useExaKeymap())
}

export function getExaModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Exa mode stack is not registered for this keymap")
  return value
}
...
export function registerExaKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createExaModeStack(keymap)
