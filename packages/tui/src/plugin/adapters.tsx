import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@exa/plugin/tui"
...
import type { useExaKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useExaKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getExaModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getExaModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
