...
import { registerExaSpinner } from "./component/register-spinner"
  ...
  const result = yield* Effect.scoped(
    Effect.gen(function* () {
        ...
        Effect.tryPromise({
          try: () =>
            createCliRenderer({
              ...
              useMouse: !Flag.EXA_DISABLE_MOUSE && input.config.mouse,
              consoleOptions: {
      ...
      yield* Effect.acquireRelease(
        Effect.sync(() => registerExaKeymap(keymap, renderer, input.config)),
                ...
                <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <ClipboardProvider>
                          <ExaKeymapProvider keymap={keymap}>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.EXA_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  renderer.console.onCopySelection = async (text: string) => {
    ...
    await clipboard
      .write?.(text)
      .then(() => toast.show({ message: copiedMessage(text), variant: "info" }))
      .catch(toast.error)
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.EXA_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("exa by Exasol")
      return
    }
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`exa | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`exa | ${route.data.id}`)
    }
  })
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.EXA_EXPERIMENTAL_WORKSPACES,
      ...
      },
      {
        ...
        title: "Open docs",
        run: () => {
          open("https://github.com/Sheetaldharshan200/exa-engine#readme").catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
  ...
  return (
    <box
      ...
      onMouseUp={
        !Flag.EXA_EXPERIMENTAL_DISABLE_COPY_ON_SELECT
          ? () => Selection.copy(renderer, toast, clipboard)
          : undefined
      }
    >
      <Show when={Flag.EXA_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
