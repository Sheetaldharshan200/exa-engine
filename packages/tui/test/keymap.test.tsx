...
import { getExaModeStack, EXA_BASE_MODE, ExaKeymapProvider, registerExaKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerExaKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when exa mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getExaModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getExaModeStack(keymap).push("autocomplete")
