...
describe("exa read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // EXA_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ exa }) =>
      Effect.gen(function* () {
        const r = yield* exa.spawn(["mcp", "list"])
        exa.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ exa }) =>
      Effect.gen(function* () {
        const r = yield* exa.spawn(["providers", "list"])
        exa.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ exa }) =>
      Effect.gen(function* () {
        const r = yield* exa.spawn(["models"])
        exa.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ exa }) =>
      Effect.gen(function* () {
        const r = yield* exa.spawn(["agent", "list"])
        exa.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh EXA_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ exa }) =>
      Effect.gen(function* () {
        const r = yield* exa.spawn(["session", "list"])
        exa.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ exa }) =>
      Effect.gen(function* () {
        const r = yield* exa.spawn(["stats"])
        exa.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under EXA_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ exa }) =>
      Effect.gen(function* () {
        const r = yield* exa.spawn(["db", "path"])
        exa.expectExit(r, 0, "db path")
