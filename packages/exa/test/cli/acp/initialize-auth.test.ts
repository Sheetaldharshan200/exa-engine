...
describe("exa acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ exa }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ exa }))
        ...
        expect(initialized.agentInfo?.name).toBe("Exa")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ exa }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ exa })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("exa-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "exa-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.EXA_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ exa }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("exa-login")
