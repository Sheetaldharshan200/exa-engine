...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ exa }) =>
    Effect.gen(function* () {
      const result = yield* exa.spawn(["--replay-limit", "10"])

      exa.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ exa }) =>
    Effect.gen(function* () {
      const result = yield* exa.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ exa }) =>
    Effect.gen(function* () {
      const result = yield* exa.spawn(["--mini", "--port", "4096"])
