...
`,
  ...
  const pure = process.env.EXA_PURE
  const meta = process.env.EXA_PLUGIN_META_FILE
  process.env.EXA_PURE = "1"
  process.env.EXA_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.EXA_PURE
    } else {
      process.env.EXA_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.EXA_PLUGIN_META_FILE
    } else {
      process.env.EXA_PLUGIN_META_FILE = meta
    }
  }
})
