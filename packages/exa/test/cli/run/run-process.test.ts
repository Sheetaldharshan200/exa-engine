// Subprocess integration tests for `exa run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `exa.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `EXA_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("exa run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, exa }) =>
      Effect.gen(function* () {
        ...
        const result = yield* exa.run("say hi")
        exa.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, exa }) =>
      Effect.gen(function* () {
        ...
        const result = yield* exa.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, exa }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* exa.run("think", { extraArgs: ["--thinking"] })
        exa.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* exa.run("think again")
        exa.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, exa }) =>
      Effect.gen(function* () {
        ...
        const result = yield* exa.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, exa }) =>
      Effect.gen(function* () {
        ...
        const result = yield* exa.run("say hi", { format: "json" })
        exa.expectExit(result, 0)

        const events = exa.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ exa }) =>
      Effect.gen(function* () {
        const result = yield* exa.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, exa }) =>
      Effect.gen(function* () {
        ...
        const result = yield* exa.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, exa }) =>
      Effect.gen(function* () {
        ...
        const result = yield* exa.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, exa }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* exa.run("request permission", { permission: { bash: "ask" } })
        exa.expectExit(denied, 0)
        ...
        const allowed = yield* exa.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        exa.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* exa.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        exa.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, exa }) =>
      Effect.gen(function* () {
        ...
        const server = yield* exa.serve()

        const result = yield* exa.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, exa }) =>
      Effect.gen(function* () {
        const result = yield* exa.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, exa }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* exa.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
