...
import { ExaClient, type GlobalEvent } from "@exa/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<ExaClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<ExaClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<ExaClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<ExaClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<ExaClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://exa.test"),
...
function sdk(
  input: {
    ...
    subscribe?: ExaClient["event"]["subscribe"]
    globalEvent?: ExaClient["global"]["event"]
    promptAsync?: ExaClient["session"]["promptAsync"]
    status?: ExaClient["session"]["status"]
    messages?: ExaClient["session"]["messages"]
    children?: ExaClient["session"]["children"]
    permissions?: ExaClient["permission"]["list"]
    questions?: ExaClient["question"]["list"]
  } = {},
) {
  const client = new ExaClient()

  const subscribe: ExaClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: ExaClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: ExaClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: ExaClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: ExaClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: ExaClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: ExaClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: ExaClient["question"]["list"] = input.questions ?? (() => ok([]))
