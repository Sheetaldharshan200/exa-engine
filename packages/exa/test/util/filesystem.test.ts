...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "exa.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "exa.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "exa.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "exa.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["exa.json", "exa.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "exa.json"),
        path.join(project, "exa.jsonc"),
      ])
    })
  })
