...
import { createExaClient } from "@exa/sdk/v2"
...
type Sdk = ReturnType<typeof createExaClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.EXA_SERVER_PASSWORD = input?.password
      Flag.EXA_SERVER_USERNAME = input?.username
