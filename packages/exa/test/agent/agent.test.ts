...
it.instance("plan agent denies edits except .exa/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".exa/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".exa", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.EXA_TEST_HOME
      process.env.EXA_TEST_HOME = test.directory
