...
Delete `packages/exa/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/exa/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/exa/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/exa`
- TUI plugin loader tests in `packages/exa` if runtime plugin API wiring changed
