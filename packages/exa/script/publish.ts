...
async function publish(dir: string, name: string, version: string) {
  ...
  // npm rate-limits publishes (v2026.1.81: 12 parallel ~55MB PUTs -> E429),
  // so retry with a backoff long enough for the window to reset.
  for (let attempt = 1; ; attempt++) {
    const r = await $`npm publish *.tgz --access public --tag ${Script.channel}`.cwd(dir).nothrow()
    if (r.exitCode === 0) return
    const out = r.stderr.toString() + r.stdout.toString()
    if (attempt >= 4 || !/E429|429 Too Many Requests/.test(out)) {
      console.error(out)
      throw new Error(`npm publish ${name}@${version} failed (attempt ${attempt})`)
    }
    console.log(`npm rate-limited publishing ${name} — retrying in 60s (attempt ${attempt})`)
...
// Sequential on purpose: parallel publishes of a dozen 50MB tarballs from
// one runner IP trip npm's rate limit.
