...
async function binPresent(bin: string): Promise<boolean> {
  try {
  ...
  try {
    switch (command) {
      ...
      case "personal_local_status": {
        const { installed, port, running } = await personalDeployment()
      ...
      case "market_env":
        return {
          ...
          value: {
            ...
            docker: await binPresent("docker"),
            podman: await binPresent("podman"),
          },
        }
      case "market_installed":
        return { ok: true, value: [] }
      case "market_detect": {
        const dep = await personalDeployment()
        return {
          ok: true,
          value: {
            "exasol-personal": dep.installed,
      ...
      case "market_catalog": {
        try {
          ...
          return { ok: true, value: res.ok ? await res.json() : null }
        } catch {
          return { ok: true, value: null }
        }
      }
