...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for exa serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(ExaHttpApi)
...
export const PublicApi = ExaHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "exa",
    version: "1.0.0",
    description: "exa api",
