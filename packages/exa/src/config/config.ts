...
function globalConfigFile() {
  const candidates = ["exa.jsonc", "exa.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      // Legacy names are still merged (installs predating the exa rename),
      // then the exa.* names take precedence.
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (Flag.EXA_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from OPENCODE_CONFIG_DIR", { path: Flag.EXA_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if ((dir.endsWith(".exa") || dir.endsWith(".opencode")) || dir === Flag.EXA_CONFIG_DIR) {
            for (const file of ["exa.json", "exa.jsonc", "opencode.json", "opencode.jsonc"]) {
