...
import { serviceUse } from "@exa/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@exa/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@exa/core/v1/config/console-state"
import { FSUtil } from "@exa/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@exa/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@exa/core/v1/config/config"
import { RemoteAuthError } from "@exa/core/v1/config/error"
import { ConfigPermissionV1 } from "@exa/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@exa/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://exasol.com/exa/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.EXA_CONFIG && !Flag.EXA_CONFIG_DIR && !Flag.EXA_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://exasol.com/exa/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "exa.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "exa.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://exasol.com/exa/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "EXA_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/exa`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://exasol.com/exa/config.json"
        ...
        if (Flag.EXA_CONFIG) {
          yield* merge(Flag.EXA_CONFIG, yield* loadFile(Flag.EXA_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.EXA_CONFIG })
        }

        if (!Flag.EXA_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("exa", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.EXA_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from EXA_CONFIG_DIR", { path: Flag.EXA_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".exa") || dir === Flag.EXA_CONFIG_DIR) {
            for (const file of ["exa.json", "exa.jsonc"]) {
          ...
          // Auto-discovered plugins under `.exa/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.EXA_CONFIG_CONTENT) {
          const source = "EXA_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.EXA_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from EXA_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["EXA_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("EXA_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.EXA_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.EXA_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("EXA_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.EXA_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.EXA_DISABLE_PRUNE) {
