...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const dirs = unique(directories).filter((dir) => (dir.endsWith(".exa") || dir.endsWith(".opencode")) || dir === Flag.EXA_CONFIG_DIR)

  for (const dir of dirs) {
    if (!(dir.endsWith(".exa") || dir.endsWith(".opencode")) && dir !== Flag.EXA_CONFIG_DIR) continue
