...
import { TuiConfig } from "@exa/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@exa/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the flat config shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.exa`
  // folders between cwd and home, and EXA_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.EXA_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit EXA_TUI_CONFIG override, if set.
  if (Flag.EXA_TUI_CONFIG) {
    const configFile = Flag.EXA_TUI_CONFIG
  ...
  // 4. `.exa` directories (and EXA_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".exa") || dir === Flag.EXA_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".exa") && dir !== Flag.EXA_CONFIG_DIR) continue
