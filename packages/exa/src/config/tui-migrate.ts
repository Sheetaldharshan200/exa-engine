...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from exa.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const exa = await exaFiles(input)
  for (const file of exa) {
...
async function exaFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "exa"),
    ...(await Filesystem.findUp(["exa.json", "exa.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "exa"))
  }
  if (Flag.EXA_CONFIG) files.push(Flag.EXA_CONFIG)
