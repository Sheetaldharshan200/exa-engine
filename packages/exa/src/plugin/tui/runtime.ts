...
import { errorData, errorMessage } from "@exa/tui/util/error"
...
import { hasTheme, upsertTheme } from "@exa/tui/context/theme"
import { Global } from "@exa/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@exa/core/util/flock"
import { Flag } from "@exa/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@exa/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@exa/core/v1/config/plugin"
import { createCommandShim } from "@exa/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@exa/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".exa"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".exa", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".exa", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.EXA_PURE ? [] : pluginOrigins
    if (Flag.EXA_PURE && pluginOrigins.length) {
    }
