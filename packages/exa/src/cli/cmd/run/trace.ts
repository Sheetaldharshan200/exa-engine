...
//
// Enable with EXA_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/exa/log/direct/<timestamp>-<pid>.jsonl. Also writes
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.EXA_DIRECT_TRACE) {
    state = false
    return undefined
  }
