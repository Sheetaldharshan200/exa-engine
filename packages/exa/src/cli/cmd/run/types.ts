// Shared type vocabulary for the direct interactive mode (`exa --mini`).
//
...
import type { ExaClient, PermissionRequest, QuestionRequest, ToolPart } from "@exa/sdk/v2"
import type { TuiConfig } from "@exa/tui/config"
...
type PromptModel = Parameters<ExaClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<ExaClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<ExaClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<ExaClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<ExaClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<ExaClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<ExaClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<ExaClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<ExaClient["question"]["reject"]>[0]
