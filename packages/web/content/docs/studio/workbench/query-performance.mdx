...
`exa web` captures the same per-run profiling baseline as the desktop app — every run's plan opens in Query Performance identically in both. Runs restored from a previous session have no baseline anywhere; run the query again and the plan is there.
</Callout>
