...
Cards reflect what is actually on this machine, not what Studio installed. An Exasol Personal set up by the `exa` CLI (or the starter kit) shows as **detected** and **running** on its card — in the desktop app and in `exa web` alike — so you are never asked to install a database that already runs.
