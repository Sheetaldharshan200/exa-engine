...
Exa supports standard proxy environment variables and custom certificates for enterprise network environments.

---

## Proxy

Exa respects standard proxy environment variables.
...
You can configure the server's port and hostname using [CLI flags](../usage/cli.mdx#run).
...
If your enterprise uses custom CAs for HTTPS connections, configure Exa to trust them.
