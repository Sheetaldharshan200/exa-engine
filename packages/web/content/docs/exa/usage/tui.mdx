...
Configured [references](../reference/references.mdx) also appear in `@` autocomplete. Type `@alias` to add the reference root as context, or type `@alias/` to autocomplete files inside that reference.
...
Most commands also have keyboard shortcuts using `ctrl+x` as the default leader key. [Learn more](../configure/keybinds.mdx).
...
Guided setup for creating or updating `AGENTS.md`. [Learn more](../configure/rules.mdx).
...
Share current session. Requires a share server configured under `enterprise.url` — see [Sharing](../configure/config.mdx#sharing).
...
- `theme` - Sets your UI theme. [Learn more](../configure/themes.mdx).
- `keybinds` - Customizes keyboard shortcuts. [Learn more](../configure/keybinds.mdx).
