---
title: Formatters
description: Exa uses language specific formatters.
---

Exa can format files after they are written or edited using language-specific formatters. Formatters are disabled by default; enable them in your config before Exa will run them.

---

## Built-in

Exa comes with several built-in formatters for popular languages and frameworks. Below is a list of the formatters, supported file extensions, and commands or config options it needs.
...
| oxfmt (Experimental) | .js, .jsx, .ts, .tsx                                                                                     | `oxfmt` dependency in `package.json` and an [experimental env variable flag](../usage/cli.mdx#experimental) |
...
When formatters are enabled, Exa will use `prettier` for matching files if your folder has `prettier` in `package.json`.

---

## How it works

When Exa writes or edits a file and formatters are enabled, it:
...
You can enable and customize formatters through the `formatter` section in your Exa config.
