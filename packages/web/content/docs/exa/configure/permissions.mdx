...
Exa uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Start Exa with `--auto` to automatically approve permission requests that are not explicitly denied.

```bash
exa --auto
```

You can also use auto mode with [`exa run`](../usage/cli.mdx#run).

```bash
exa run --auto "Refactor this module"
```
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where Exa was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many `bash` commands).
...
Exa permissions are keyed by tool name, plus a couple of safety guards:
...
- `external_directory` — triggered when a tool touches paths outside the folder working directory
...
If you don’t specify anything, Exa starts from permissive defaults:
...
When Exa prompts for approval, the UI offers three outcomes:

- `once` — approve just this request
- `always` — approve future requests matching the suggested patterns (for the rest of the current Exa session)
...
You can override permissions per agent. Agent permissions are merged with the global config, and agent rules take precedence. [Learn more](./agents.mdx#permissions) about agent permissions.
