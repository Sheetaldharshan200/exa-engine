...
There are two types of agents in Exa; primary agents and subagents.
...
Exa comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Exa comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

Exa comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into Exa's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `exa.json` config file:
...
- Global: `~/.config/exa/agents/`
- Per-folder: `.exa/agents/`

```markdown title="~/.config/exa/agents/review.md"
---
...
If no temperature is specified, Exa uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Exa config and the folder specific config.
...
<Callout type="info">
If you don’t specify a model, primary agents use the [model globally configured](./config.mdx#models) while subagents will use the model of the primary agent that invoked the subagent.
</Callout>
...
The model ID in your Exa config uses the format `provider/model-id`. For example, with Anthropic you would use `anthropic/claude-sonnet-5`.
...
[Learn more about tools](./tools.mdx).
...
| `bash`               | `bash`                                                           |
| `task`               | `task`                                                           |
| `external_directory` | Any tool that reads or writes files outside the folder worktree |
...
<Callout type="info">
Run `exa models` to see a list of the available models.
</Callout>
...
1. Ask where to save the agent; global or folder-specific.
...
<Callout type="info">
Do you have an agent you'd like to share? [Submit a PR](https://github.com/Sheetaldharshan200/exa-engine).
</Callout>
...
```markdown title="~/.config/exa/agents/docs-writer.md"
---
...
```markdown title="~/.config/exa/agents/security-auditor.md"
---
