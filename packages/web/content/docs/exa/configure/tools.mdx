...
Tools allow the LLM to perform actions in your codebase. Exa comes with a set of built-in tools, but you can extend it with [custom tools](./custom-tools.mdx) or [MCP servers](./mcp-servers.mdx).

By default, all tools are **enabled** and don't need permission to run. You can control tool behavior through [permissions](./permissions.mdx).
...
[Learn more](./permissions.mdx) about configuring permissions.

---

## Built-in

Here are all the built-in tools available in Exa.

---

### bash

Execute shell commands in your folder environment.
...
<Callout type="info">
This tool is only available when `EXA_EXPERIMENTAL_LSP_TOOL=true` (or `EXA_EXPERIMENTAL=true`).
</Callout>
...
To configure which LSP servers are available for your folder, see [LSP Servers](./lsp.mdx).
...
`apply_patch` uses `output.args.patchText` instead of `output.args.filePath`. Paths are embedded in marker lines within `patchText` and are relative to the folder root (for example: `*** Add File: src/new-file.ts`, `*** Update File: src/existing.ts`, `*** Move to: src/renamed.ts`, `*** Delete File: src/obsolete.ts`).
...
Load a [skill](./skills.mdx) (a `SKILL.md` file) and return its content in the conversation.
...
<Callout type="info">
This tool is disabled for subagents by default, but you can enable it manually. [Learn more](./agents.mdx#permissions)
</Callout>
...
<Callout type="info">
This tool is only available when using the Exa provider or when the `EXA_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Exa:

```bash
EXA_ENABLE_EXA=1 exa
```
...
[Learn more](./custom-tools.mdx) about creating custom tools.
...
[Learn more](./mcp-servers.mdx) about configuring MCP servers.
...
To include files that would normally be ignored, create a `.ignore` file in your folder root. This file can explicitly allow certain paths.
