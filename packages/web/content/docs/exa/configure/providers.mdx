---
title: Providers
description: Using any LLM provider in Exa.
---
...
2. Configure the provider in your Exa config.
...
in `~/.local/share/exa/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Exa
config.
...
Exa can run models without any provider account at all — its built-in engine downloads and serves curated models on your machine, and a running Ollama is detected automatically. Both appear in `/connect` and `/models` like any other provider. See [Local models](../data/local-models.mdx).
...
To use Amazon Bedrock with Exa:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running exa:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY exa

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile exa

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX exa
   ```
   ...
   ***
   ...
   For folder-specific or persistent configuration, use `exa.json`:
...
Sign in with your **Claude Pro or Max subscription** — no API key, usage bills to the plan you already pay for. Or use an API key from the Anthropic Console.

1. Run the `/connect` command and select Anthropic (also available from the terminal as `exa providers login`).
...
2. Pick an auth method:
   ...
   - **Claude Pro/Max (subscription)** — your browser opens claude.ai; sign in, then paste the code shown on the final page back into Exa.
   - **Console account (creates an API key)** — sign in to the Anthropic Console and Exa creates and stores an API key for you (billed per token).
   - **API key (manual)** — paste an existing key.

3. All Anthropic models are now available in the `/models` command.

   ```txt
   /models
   ```

Tokens refresh automatically; you sign in once. To keep several Anthropic accounts and switch between them, see [`exa accounts`](../usage/cli.mdx#accounts).

---

### Atomic Chat

You can configure exa to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   <Callout type="info">
   The deployment name must match the model name for exa to work properly.
   </Callout>
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX exa
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your exa config.
...
Exa supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Exa uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Exa. Sign in and approve.

   <Callout type="info">
   Exa requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   </Callout>
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Exa surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Exa forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Exa is experimental. Features, configuration, and
behavior may change in future releases.
</Callout>

Exa integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
providing AI-powered agentic chat with native tool calling capabilities.

<Callout type="info" title="License requirements">
...
to store token in exa auth storage.
</Callout>

##### Self-Hosted GitLab

<Callout type="info" title="compliance note">
Exa uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Exa
to only use your own GitLab-hosted instance, add the following to your
`exa.json` file. It is also recommended to disable session sharing.
...
When a `duo-workflow-*` model is selected, Exa will:
...
4. Route tool execution requests through Exa's permission-gated tool system
...
To use your GitHub Copilot subscription with exa:
...
To use Google Vertex AI with Exa:
...
2. Set the required environment variables:
   ...
   Set them while running exa.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id exa
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through exa, you can always configure it yourself.
...
```jsonc title="~/.config/exa/exa.jsonc"
{
  "$schema": "https://raw.githubusercontent.com/Sheetaldharshan200/exa-engine/main/schema/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [exa-helicone-session](https://github.com/H2Shami/exa-helicone-session) plugin to automatically log each Exa conversation as a session in Helicone.

```bash
npm install -g exa-helicone-session
```
...
```json title="exa.json"
{
  "plugin": ["exa-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Exa conversation listed as a separate session.
...
You can configure exa to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure exa to use local models through LM Studio.
...
You can configure exa to use local models through Ollama.

<Callout type="info">
Ollama can automatically configure itself for Exa. See the [Ollama integration docs](https://docs.ollama.com/integrations/exa) for details.
</Callout>
...
3. Copy the API key for use in Exa.
...
6. **Important**: Before using cloud models in Exa, you must pull the model information locally:
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your exa config.
...
5. You can also customize them through your exa config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' exa
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group exa
   ```
...
1. Head over to [STACKIT Portal](https://portal.stackit.cloud), navigate to **AI Model Serving**, and create an auth token for your folder.

   <Callout type="info">
   You need a STACKIT customer account, user account, and folder before creating auth tokens.
   </Callout>
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Exa:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=exa&utm_medium=docs&utm_campaign=cortex-provider).

exa's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. Exa captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting exa:
...
The model catalog is provided automatically. A minimal `exa.json` is all that's needed:
...
You can also customize models through your exa config. Here's an example of specifying provider routing order.
...
2. Select **SuperGrok Subscription**. Exa opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by Exa. Exa polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
Exa refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
<Callout type="info">
You can use any OpenAI-compatible provider with exa. Most modern AI providers offer OpenAI-compatible APIs.
</Callout>
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in exa.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `exa.json` file in your folder directory:
...
- **apiKey**: Set using `env` variable syntax, [learn more](./config.mdx#env-vars).
...
The `limit` fields allow Exa to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `exa auth list` to see if the credentials
...
2. For custom providers, check the exa config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your exa config.
