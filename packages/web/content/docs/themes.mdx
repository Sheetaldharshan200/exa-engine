...
With Exa you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Exa uses our own `exa` theme.
...
Exa comes with several built-in themes.
...
- Want Exa to match their terminal's appearance
...
Exa supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/exa/themes/*.json` or `$XDG_CONFIG_HOME/exa/themes/*.json`
3. **Folder root directory** - Defined in the `<project-root>/.exa/themes/*.json`
4. **Current working directory** - Defined in `./.exa/themes/*.json`
...
And for folder-specific themes.
