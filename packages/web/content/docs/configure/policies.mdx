---
title: Policies
description: Control which configured resources Exa may use.
---

Policies control whether Exa may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `exa.json`.

Policies are separate from [permissions](./permissions.mdx). Permissions control what tools can do during a session, while policies control whether Exa may use a resource such as an LLM provider.
...
Exa currently supports one policy action:
...
Policies may be set in both your global config and folder config. If policies from both locations match the same provider, your global policy takes priority over the folder policy. This prevents a repository from re-enabling a provider that you deny globally.
