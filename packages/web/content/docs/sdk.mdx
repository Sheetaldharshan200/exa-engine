---
title: SDK
description: Type-safe JS client for exa server.
---

export const typesUrl = "https://github.com/Sheetaldharshan200/exa-engine/blob/main/packages/sdk/js/src/gen/types.gen.ts"

The exa JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control exa programmatically.

[Learn more](./server.mdx) about how the server works. For examples, check out the [integrations](./ecosystem.mdx) built on it.

---

## Install

The SDK is published on npm as **`exa-engine-sdk`**. Install it under its internal alias so every import in these docs works verbatim:

```bash
npm install @exa/sdk@npm:exa-engine-sdk
```

Or install it under its published name and import from `"exa-engine-sdk"` instead:

```bash
npm install exa-engine-sdk
```
...
Create an instance of exa:

```javascript
import { createExa } from "@exa/sdk"

const { client } = await createExa()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `exa.json`, but you can override or add configuration inline:

```javascript
import { createExa } from "@exa/sdk"

const exa = await createExa({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${exa.server.url}`)
...
If you already have a running instance of exa, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@exa/sdk"
```
...
| Method              | Description         | Response                                      |
| ------------------- | ------------------- | --------------------------------------------- |
| `project.list()`    | List all folders   | <a href={typesUrl}><code>Project[]</code></a> |
| `project.current()` | Get current folder | <a href={typesUrl}><code>Project</code></a>   |
...
- `directory`: override the folder root for the search
