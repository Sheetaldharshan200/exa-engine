---
title: Server
description: Interact with exa server over HTTP.
---

export const typesUrl = "https://github.com/Sheetaldharshan200/exa-engine/blob/main/packages/sdk/js/src/gen/types.gen.ts"

The `exa serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an exa client can use.
...
```bash
exa serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Custom domain name for mDNS service | `exa.local` |
| `--cors`        | Additional browser origins to allow | `[]`             |

`--cors` can be passed multiple times:

```bash
exa serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `EXA_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `exa`, or set `EXA_SERVER_USERNAME` to override it. This applies to both `exa serve` and `exa web`.

```bash
EXA_SERVER_PASSWORD=your-password exa serve
```
...
When you run `exa` it starts a TUI and a server. Where the TUI is the
client that talks to the server. The server exposes an OpenAPI 3.1 spec
endpoint. This endpoint is also used to generate an [SDK](./sdk.mdx).

<Callout type="info">
Use the exa server to interact with exa programmatically.
</Callout>

This architecture lets exa support multiple clients and allows you to interact with exa programmatically.

You can run `exa serve` to start a standalone server. If you have the
exa TUI running, `exa serve` will start a new server.

---

#### Connect to an existing server

When you start the TUI it randomly assigns a port and hostname. You can instead pass in the `--hostname` and `--port` [flags](../usage/cli.mdx). Then use this to connect to its server.

The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Exa [IDE](../usage/ide.mdx) plugins.
...
The exa server exposes the following APIs.
...
| Method | Path               | Description             | Response                                      |
| ------ | ------------------ | ----------------------- | --------------------------------------------- |
| `GET`  | `/project`         | List all folders       | <a href={typesUrl}><code>Folder[]</code></a> |
| `GET`  | `/project/current` | Get the current folder | <a href={typesUrl}><code>Folder</code></a>   |
...
| `GET`  | `/vcs`  | Get VCS info for the current folder | <a href={typesUrl}><code>VcsInfo</code></a> |
...
- `directory` (optional) — override the folder root for the search
