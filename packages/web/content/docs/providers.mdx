...
Exa can run models without any provider account at all — its built-in engine downloads and serves curated models on your machine, and a running Ollama is detected automatically. Both appear in `/connect` and `/models` like any other provider. See [Local models](./local-models.mdx).
...
Tokens refresh automatically; you sign in once. To keep several Anthropic accounts and switch between them, see [`exa accounts`](./cli.mdx#accounts).
