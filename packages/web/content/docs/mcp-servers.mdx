...
You can add external tools to Exa using the _Model Context Protocol_, or MCP. Exa supports both local and remote servers.
...
You can define MCP servers in your [Exa Config](./config.mdx) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/exa` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Your local config values override the remote defaults. See [config precedence](./config.mdx#precedence-order) for more details.
...
Exa automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Exa will:
...
If the server requires authentication, Exa will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `exa mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Exa will store the tokens securely in `~/.local/share/exa/mcp-auth.json`.
...
Your MCPs are available as tools in Exa, alongside built-in tools. So you can manage them through the Exa config like any other tool.
...
2. In your [agent config](./agents.mdx#tools), enable the MCP server as a tool.
...
Add the [Sentry MCP server](https://mcp.sentry.dev) to interact with your Sentry folders and issues.
...
This will open a browser window to complete the OAuth flow and connect Exa to your Sentry account.

Once authenticated, you can use Sentry tools in your prompts to query issues, folders, and error data.
...
Alternatively, you can add something like this to your [AGENTS.md](./rules.mdx).
