---
title: Plugins
description: Écrivez vos propres plugins pour étendre Exa.
---

Les plugins vous permettent d'étendre Exa en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Exa.
...
- `.exa/plugins/` - Plugins au niveau du projet
- `~/.config/exa/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/exa/node_modules/`.
...
1. Configuration globale (`~/.config/exa/exa.json`)
2. Configuration du projet (`exa.json`)
3. Répertoire global des plugins (`~/.config/exa/plugins/`)
4. Répertoire des plugins du projet (`.exa/plugins/`)
...
Exa exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK exa pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre exa.
...
:::note
Si vous utilisez l'application de bureau Exa, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez exa de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à exa :
...
L'assistant `tool` crée un outil personnalisé que exa peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour exa aux côtés des outils intégrés.
