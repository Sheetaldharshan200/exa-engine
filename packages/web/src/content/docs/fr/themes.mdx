...
Avec Exa, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Exa utilise notre propre thème `exa`.
...
Exa est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Exa corresponde à l'apparence de leur terminal
...
Exa prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/exa/themes/*.json` ou `$XDG_CONFIG_HOME/exa/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.exa/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.exa/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/exa/themes
vim ~/.config/exa/themes/my-theme.json
```
