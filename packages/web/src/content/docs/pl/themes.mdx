...
Dzięki exa możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie exa używa naszego własnego motywu `exa`.
...
exa ma kilka wbudowanych motywów.
...
- Chcą, aby Exa pasował do wyglądu ich terminala
...
exa obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/exa/themes/*.json` lub `$XDG_CONFIG_HOME/exa/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.exa/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.exa/themes/*.json`
