...
Możesz dodać dodatkowe informacje do exa za pomocą _Model Context Protocol_, w skrócie MCP. exa obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [exa Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/exa`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
exa automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, exa:
...
Jeśli serwer wymaga uwierzytelnienia, exa poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `exa mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji exa bezpieczne przechowa tokeny w `~/.local/share/exa/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w exa, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie exa, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć exa z kontem Sentry.
