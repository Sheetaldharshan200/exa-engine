...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Exa posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Exa.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `EXA_EXPERIMENTAL_LSP_TOOL=true` (lub `EXA_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Exa lub gdy zmienna środowiskowa `EXA_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Exa:
