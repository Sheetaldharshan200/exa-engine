...
Umiejętności agenta pozwalają exa odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
exa przeszukuje te lokalizacje:

- Project config: `.exa/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu exa przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.exa/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/exa/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.exa/skills/git-release/SKILL.md` w ten sposób:
...
exa wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `exa.json`:
...
**Dla agentów wbudowanych** (w `exa.json`):
