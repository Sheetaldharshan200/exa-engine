---
title: Serwer
description: Interakcja z serwerem exa za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `exa serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient exa.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `exa.local` |
...
Ustaw `EXA_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `exa` lub ustaw `EXA_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `exa serve`, jak i `exa web`.
...
Kiedy uruchomisz `exa`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera exa do programowej interakcji z exa.
:::

Ta architektura umożliwia obsługę wielu klientów przez exa i programową interakcję z kodem otwartym.

Możesz uruchomić `exa serve`, aby uruchomić samodzielny serwer. Jeśli masz
exa TUI działa, `exa serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki exa [IDE](/docs/ide).
...
Serwer exa udostępnia następujące interfejsy API.
