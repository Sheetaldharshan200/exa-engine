---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć exa.
---

Wtyczki umożliwiające wyłączenie exa poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania exa.
...
- `.exa/plugins/` - Wtyczki na poziomie projektu
- `~/.config/exa/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/exa/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/exa/exa.json`)
2. Konfiguracja projektu (`exa.json`)
3. Globalny katalog wtyczek (`~/.config/exa/plugins/`)
4. Katalog wtyczek projektu (`.exa/plugins/`)
...
exa uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z exa do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do exa.
...
:::note
Jeśli korzystasz z aplikacji komputerowej exa, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj exa czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do exa:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod exa. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
