...
exa ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak exa kendi `exa` temasını kullanır.
...
exa birden fazla yerleşik temayla gelir.
...
- exa'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
exa, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/exa/themes/*.json` veya `$XDG_CONFIG_HOME/exa/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.exa/themes/*.json`
4. **Geçerli çalışma dizini** - `./.exa/themes/*.json`
