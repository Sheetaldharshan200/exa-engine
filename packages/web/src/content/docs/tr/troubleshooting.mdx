...
exa ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\exa\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `exa --log-level DEBUG`.

---

## Depolama

exa, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/exa/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\exa` yapistirin
...
exa Desktop arka planda yerel bir exa sunucusu (`exa-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `exa` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/exa/exa.jsonc` (veya `~/.config/exa/exa.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/exa/exa.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\exa\exa.jsonc` yapistirin
...
exa diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/exa/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\exa\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), exa'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. exa Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/exa`
- **Linux**: `~/.cache/exa` dizinini silin (veya `rm -rf ~/.cache/exa` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\exa` yapistirin

3. exa Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

exa Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`exa.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `EXA_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `EXA_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta exa Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, exa ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

exa Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda exa icin bildirimler etkinse
...
1. exa Desktop'u kapatin
2. Su dosyalari bulun ve silin (exa Desktop uygulama veri dizininde yer alirlar):

- `exa.settings.dat` (desktop varsayilan sunucu URL'si)
- `exa.global.dat` ve `exa.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
exa ile ilgili bir sorun yasiyorsaniz:
...
3. `exa upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `exa models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\exa`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. exa, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\exa`

2. En guncel provider paketlerini yeniden kurmak icin exa'u yeniden baslatin

Bu, exa'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
exa, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
