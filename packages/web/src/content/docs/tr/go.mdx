...
Exa Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Exa'daki diğer sağlayıcılar gibi çalışır. Exa Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Exa'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Exa Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Exa Go, Exa'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Exa Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Exa Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Exa Go'ya abone olabilir.
:::
...
Exa Go aşağıdaki limitleri içerir:
...
Exa yapılandırmanızdaki [model id](/docs/config/#models) formatı `exa-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `exa-go/kimi-k3` kullanmalısınız.
...
Exa Go'yu şu amaçlarla oluşturduk:
...
4. Exa ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
