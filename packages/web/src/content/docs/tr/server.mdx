---
title: Sunucu
description: exa sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`exa serve` komutu, exa istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `exa.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `EXA_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `exa` değeridir; değiştirmek isterseniz `EXA_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `exa serve` hem de `exa web` için geçerlidir.
...
`exa` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
exa ile programatik etkileşim için exa sunucusunu kullanın.
:::

Bu mimari, exa'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`exa serve` ile bağımsız bir sunucu başlatabilirsiniz. exa TUI açıksa, `exa serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum exa [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
exa sunucusu aşağıdaki API'leri sunar.
