---
title: Eklentiler
description: Exa'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Exa'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Exa'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.exa/plugins/` - Proje düzeyinde eklentiler
- `~/.config/exa/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/exa/node_modules/`'da önbelleğe alınır.
...
3. Global eklenti dizini (`~/.config/exa/plugins/`)
4. Proje eklenti dizini (`.exa/plugins/`)
...
Exa bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Exa SDK istemcisi.
...
Exa'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Exa masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Exa'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Exa'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Exa'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Exa'a da hazır olacaktır.
