...
_Model Bağlam Protokolü_ veya MCP kullanarak exa'a harici araçlar ekleyebilirsiniz. exa hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [exa Config](https://opencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/exa` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
exa, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde exa şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa exa, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `exa mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından exa, belirteçleri `~/.local/share/exa/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra exa'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi exa yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve exa'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
