...
description: Exa, LSP sunucularınızla bütünleşir.
---

Exa, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Exa, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`EXA_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve exa bir dosyayı açtığında:
...
LSP sunucularını, exa yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
