...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. exa yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
exa içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `EXA_EXPERIMENTAL_LSP_TOOL=true` (veya `EXA_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca exa provider kullanılırken veya `EXA_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

exa başlatırken etkinleştirmek için:
