---
title: Sağlayıcılar
description: exa'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

exa, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı exa yapılandırmanızda yapılandırın.
...
`~/.local/share/exa/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları exa'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Exa Zen, exa ekibi tarafından sağlanan modellerin bir listesidir.
exa ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Exa Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, exa'u seçin ve [opencode.ai/auth](https://opencode.ai/auth)'ye gidin.
...
exa'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Exa Go

Exa Go, exa ile iyi çalıştığı test edilmiş ve doğrulanmış, exa ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Exa Go`'yu seçin ve [opencode.ai/auth](https://opencode.ai/zen) adresine gidin.
...
Amazon Bedrock'u exa ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   exa'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `exa.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi exa'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
exa'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   exa'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
exa, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. exa'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`exa.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`exa.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi exa'la kullanmak için:
...
Google Vertex AI'yi exa ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları exa çalıştırılırken ayarlayın.
...
Helicone'un exa aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her exa görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [exa-helicone-session](https://github.com/H2Shami/exa-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her exa görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
exa'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla exa'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla exa'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini exa için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/exa)'a bakın.
:::
...
Ollama Cloud'u exa ile kullanmak için:
...
3. exa'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: exa'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Exa Zen, exa ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Exa Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Exa Zen**'i arayın.

   ```txt
   /connect
   ```

3. exa API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   exa yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca exa yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri exa yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı exa ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `exa.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları exa'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `exa auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için exa yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, exa yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
