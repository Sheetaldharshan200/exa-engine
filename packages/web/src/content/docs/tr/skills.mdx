...
Ajan becerileri, exa'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
exa su konumlari tarar:

- Proje konfigurasyonu: `.exa/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/exa/skills/<name>/SKILL.md`
...
Proje yerel yollarinda exa, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.exa/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/exa/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.exa/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
exa, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`exa.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`exa.json` icinde):
