...
Du kan legge til eksterne verktøy i Exa ved å bruke _Model Context Protocol_ eller MCP. Exa støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Exa Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="exa.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/exa`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Exa håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Exa:
...
Hvis serveren krever autentisering, vil Exa be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `exa mcp auth <server-name>`.
...
```bash
exa mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Exa lagre tokens sikkert i `~/.local/share/exa/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i Exa, sammen med innebygde verktøy. Så du kan administrere dem gjennom Exa-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Exa til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Exa-samtale oppført som en separat økt.
