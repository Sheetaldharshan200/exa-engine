---
title: Tjener
description: Samhandle med Exa-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `exa serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Exa-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `exa.local` |
...
Sett `EXA_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `exa` som standard, eller sett `EXA_SERVER_USERNAME` for å overstyre det. Dette gjelder både `exa serve` og `exa web`.
...
Når du kjører `exa` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Exa-serveren til å samhandle med Exa programmatisk.
:::

Denne arkitekturen lar Exa støtte flere klienter og lar deg samhandle med Exa programmatisk.

Du kan kjøre `exa serve` for å starte en frittstående server. Hvis du har
Exa TUI kjører, vil `exa serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Exa [IDE](/docs/ide) plugins.
...
exa-serveren viser følgende APIer.
