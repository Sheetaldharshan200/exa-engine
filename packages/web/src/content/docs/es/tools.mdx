...
Las herramientas permiten que LLM realice acciones en su código base. Exa viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Exa.
...
:::note
Esta herramienta solo está disponible cuando `EXA_EXPERIMENTAL_LSP_TOOL=true` (o `EXA_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Exa o cuando la variable de entorno `EXA_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Exa:
