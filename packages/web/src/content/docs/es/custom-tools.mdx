...
description: Cree herramientas que LLM pueda llamar en exa.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de exa como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.exa/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/exa/tools/`.
...
```ts title=".exa/tools/database.ts" {1}
...
```ts title=".exa/tools/project.ts" {8}
import { tool } from "@exa/plugin"
...
```ts title=".exa/tools/python-add.ts" {10}
import { tool } from "@exa/plugin"
