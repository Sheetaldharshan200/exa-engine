---
title: Complementos
description: Escriba sus propios complementos para extender Exa.
---

Los complementos le permiten extender Exa al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Exa.
...
- `.exa/plugins/` - Complementos a nivel de proyecto
- `~/.config/exa/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/exa/node_modules/`.
...
1. Configuración global (`~/.config/exa/exa.json`)
2. Configuración del proyecto (`exa.json`)
3. Directorio global de complementos (`~/.config/exa/plugins/`)
4. Directorio de complementos del proyecto (`.exa/plugins/`)
...
Exa ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK exa para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar exa.
...
:::note
Si está utilizando la aplicación de escritorio Exa, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que exa lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a exa:
...
El ayudante `tool` crea una herramienta personalizada a la que exa puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para exa junto con las herramientas integradas.
