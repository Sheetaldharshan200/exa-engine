---
title: Servidor
description: Interactuar con el servidor exa a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `exa serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente exa puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `exa.local` |
...
Configure `EXA_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `exa`, o configure `EXA_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `exa serve` como a `exa web`.
...
Cuando ejecuta `exa`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor exa para interactuar con exa mediante programación.
:::

Esta arquitectura permite que exa admita múltiples clientes y le permite interactuar con exa mediante programación.

Puede ejecutar `exa serve` para iniciar un servidor independiente. Si tienes el
exa TUI ejecutándose, `exa serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Exa [IDE](/docs/ide).
...
El servidor exa expone las siguientes API.
