...
Con Exa puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Exa usa nuestro propio tema `exa`.
...
Exa viene con varios temas integrados.
...
- Quiere que Exa coincida con la apariencia de su terminal
...
Exa admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/exa/themes/*.json` o `$XDG_CONFIG_HOME/exa/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.exa/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.exa/themes/*.json`
