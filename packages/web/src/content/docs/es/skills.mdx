...
Las habilidades del agente permiten a Exa descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Exa busca estas ubicaciones:

- Configuración del proyecto: `.exa/skills/<name>/SKILL.md`
- Configuración global: `~/.config/exa/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Exa sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.exa/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/exa/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.exa/skills/git-release/SKILL.md` así:
...
Exa enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `exa.json`:
...
**Para agentes integrados** (en `exa.json`):
