...
Med Exa kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Exa vårt eget `exa`-tema.
...
Exa støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/exa/themes/*.json` eller `$XDG_CONFIG_HOME/exa/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.exa/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.exa/themes/*.json`
