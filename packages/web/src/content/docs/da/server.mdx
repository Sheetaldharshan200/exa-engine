---
title: Server
description: Interager med exa-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `exa serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en exa-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `exa.local` |
...
Indstil `EXA_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `exa`, eller indstil `EXA_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `exa serve` og `exa web`.
...
Når du kører `exa` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug exa-serveren til at interagere med exa programmatisk.
:::

Denne arkitektur lader exa understøtte flere klienter og lader dig interagere med exa programmatisk.

Du kan køre `exa serve` for at starte en selvstændig server. Hvis du har
exa TUI kørende, vil `exa serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Exa [IDE](/docs/ide) plugins.
...
Exa-serveren viser følgende API'er.
