---
title: Plugins
description: Skriv dine egne plugins for at udvide Exa.
---

Plugins giver dig mulighed for at udvide Exa ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Exas standardadfærd.
...
- `.exa/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/exa/node_modules/`.
...
1. Global konfiguration (`~/.config/exa/exa.json`)
2. Projektkonfiguration (`exa.json`)
3. Global plugin-mappe (`~/.config/exa/plugins/`)
4. Projekt plugin bibliotek (`.exa/plugins/`)
...
Exa kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En exa SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide exa.
...
:::note
Hvis du bruger Exa desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Plugins kan også tilføje brugerdefinerede værktøjer til exa:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som exa kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for exa sammen med indbyggede værktøjer.
