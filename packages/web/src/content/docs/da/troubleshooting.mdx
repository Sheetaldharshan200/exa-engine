...
For at fejlfinde problemer med Exa, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\exa\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `exa --log-level DEBUG`.

---

## Lagring

exa gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/exa/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\exa`
...
Exa Desktop kører en lokal Exa-server (`exa-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Exa`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/exa/exa.jsonc` (eller `~/.config/exa/exa.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/exa/exa.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\exa\exa.jsonc`
...
Exa kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/exa/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\exa\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Exa kan genopbygge den.

1. Afslut Exa Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/exa`
- **Linux**: slet `~/.cache/exa` (eller kør `rm -rf ~/.cache/exa`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\exa`

3. Start Exa Desktop på ny.

---

### Løs problemer med serverforbindelse

Exa Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `exa.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `EXA_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `EXA_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Exa Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Exa.

---

### Meddelelser vises ikke

Exa Desktop viser kun systemvarsler når:

- varsler er aktiveret for Exa i dine OS-indstillinger, og
...
1. Afslut Exa Desktop.
2. Find og slet disse filer (de findes i Exa Desktop-appens datamappe):

- `exa.settings.dat` (skrivebordsstandardserver URL)
- `exa.global.dat` og `exa.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Exa:
...
3. Sørg for at du har den nyeste version med `exa upgrade`
...
For at finde ud af hvilke modeller du har adgang til, kør `exa models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\exa`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. exa installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\exa`

2. Start exa på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge exa til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
exa vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
