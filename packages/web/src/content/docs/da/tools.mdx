...
Her er alle de innebygde verktøyene som er tilgængelige i Exa.
...
:::note
Dette verktøyet er bare tilgængelig når `EXA_EXPERIMENTAL_LSP_TOOL=true` (eller `EXA_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Exa-leverandøren eller når miljøvariabelen `EXA_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
