---
title: サーバー
description: HTTP 経由で Exa サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`exa serve` コマンドは、Exa クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `exa.local` |
...
HTTP 基本認証でサーバーを保護するには、`EXA_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `exa` になるか、`EXA_SERVER_USERNAME` を設定してオーバーライドします。これは、`exa serve` と `exa web` の両方に当てはまります。
...
`exa` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Exa サーバーを使用して、プログラムで Exa と対話します。
:::

`exa serve` を実行してスタンドアロンサーバーを起動できます。すでに Exa TUI を実行している場合でも、`exa serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Exa [IDE](/docs/ide) プラグインによって使用されます。
