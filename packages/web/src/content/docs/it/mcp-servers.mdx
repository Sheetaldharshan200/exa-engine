...
Puoi aggiungere strumenti esterni a Exa usando il _Model Context Protocol_, o MCP. Exa supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Exa Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/exa`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Exa gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Exa:
...
Se il server richiede autenticazione, Exa ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `exa mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Exa memorizzera' i token in modo sicuro in `~/.local/share/exa/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Exa insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Exa come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Exa al tuo account Sentry.
