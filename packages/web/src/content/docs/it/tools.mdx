...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. Exa include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in Exa.
...
:::note
Questo strumento e' disponibile solo quando `EXA_EXPERIMENTAL_LSP_TOOL=true` (oppure `EXA_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider Exa o quando la variabile d'ambiente `EXA_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di Exa:
