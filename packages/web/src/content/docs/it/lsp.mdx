---
title: Server LSP
description: Exa si integra con i tuoi server LSP.
---

Exa può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

Exa include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `EXA_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e exa apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di exa.
