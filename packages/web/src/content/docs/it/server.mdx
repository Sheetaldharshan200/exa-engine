---
title: Server
description: Interagisci con il server exa via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `exa serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client exa.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `exa.local` |
...
Imposta `EXA_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `exa`, oppure imposta `EXA_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `exa serve` sia per `exa web`.
...
Quando esegui `exa` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server exa per interagire con exa in modo programmatico.
:::

Questa architettura permette a exa di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `exa serve` per avviare un server standalone. Se la TUI di exa e gia in esecuzione, `exa serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Exa.
...
Il server exa espone le seguenti API.
