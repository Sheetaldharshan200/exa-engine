...
Le skill degli agenti permettono a Exa di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Exa cerca in queste posizioni:

- Config di progetto: `.exa/skills/<name>/SKILL.md`
- Config globale: `~/.config/exa/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Exa risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.exa/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/exa/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.exa/skills/git-release/SKILL.md` cosi':
...
Exa elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `exa.json`:
...
**Per agenti integrati** (in `exa.json`):
