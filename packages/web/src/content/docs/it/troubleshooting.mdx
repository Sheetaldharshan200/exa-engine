...
Per diagnosticare problemi con Exa, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\exa\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `exa --log-level DEBUG`.

---

## Archiviazione

exa salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/exa/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\exa`
...
Exa Desktop esegue in background un server locale di Exa (il sidecar `exa-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Exa` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/exa/exa.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\exa\exa.jsonc`
...
Exa puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/exa/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\exa\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Exa possa ricostruirla.

1. Chiudi completamente Exa Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/exa`
- **Linux**: elimina `~/.cache/exa` (oppure esegui `rm -rf ~/.cache/exa`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\exa`

3. Riavvia Exa Desktop.

---

### Risolvi problemi di connessione al server

Exa Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `exa.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `EXA_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `EXA_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, Exa Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Exa.

---

### Notifiche non visualizzate

Exa Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Exa nelle impostazioni del sistema operativo, e
...
1. Chiudi Exa Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Exa Desktop):

- `exa.settings.dat` (desktop default server URL)
- `exa.global.dat` e `exa.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
3. Assicurati di avere l'ultima versione con `exa upgrade`
...
Per capire a quali modelli hai accesso, esegui `exa models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\exa`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. exa installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\exa`

2. Riavvia exa per reinstallare i pacchetti provider piu recenti

Questo forzera exa a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
exa rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
