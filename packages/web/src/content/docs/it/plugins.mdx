---
title: Plugin
description: Scrivi plugin per estendere Exa.
---

I plugin ti permettono di estendere Exa agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Exa.
...
- `.exa/plugins/` - plugin a livello progetto
- `~/.config/exa/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/exa/node_modules/`.
...
1. Config globale (`~/.config/exa/exa.json`)
2. Config di progetto (`exa.json`)
3. Directory plugin globale (`~/.config/exa/plugins/`)
4. Directory plugin di progetto (`.exa/plugins/`)
...
Exa esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di exa per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere exa.
...
:::note
Se usi l'app desktop di Exa, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a exa di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a exa:
...
L'helper `tool` crea uno strumento personalizzato che exa può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in exa insieme agli strumenti integrati.
