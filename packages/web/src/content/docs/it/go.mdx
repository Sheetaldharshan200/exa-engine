...
Exa Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Exa. Ti abboni a Exa Go e
...
Exa Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Exa Go funziona come qualsiasi altro provider in Exa.

1. Accedi a **<a href={console}>Exa Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Exa Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Exa Go.
:::
...
Exa Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Exa config
utilizza il formato `exa-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`exa-go/kimi-k3` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Exa.
