---
title: Supporto ACP
description: Usa Exa in qualunque editor compatibile con ACP.
---

Exa supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare Exa via ACP, configura l'editor in modo che esegua il comando `exa acp`.

Il comando avvia Exa come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'Exa' nel selettore agenti di AI Chat.
...
Per usare Exa come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare Exa come agente ACP per la chat.

Se devi passare variabili d'ambiente (come `EXA_API_KEY`), vedi [Configuring Adapters: Environment Variables](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) nella documentazione di CodeCompanion.nvim.

## Supporto

Exa funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione Exa
