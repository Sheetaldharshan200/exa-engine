...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Exa dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Exa.
...
:::note
Ovaj alat je dostupan samo kada je `EXA_EXPERIMENTAL_LSP_TOOL=true` (ili `EXA_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Exa provajdera ili kada je varijabla `EXA_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Exa:
