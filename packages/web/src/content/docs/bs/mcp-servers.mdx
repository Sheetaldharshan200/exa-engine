...
Možete dodati vanjske alate u Exa koristeći _Model Context Protocol_, ili MCP. Exa podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Exa Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/exa`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Exa automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Exa će:
...
Ako server zahtijeva autentifikaciju, Exa će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `exa mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Exa sigurno čuva tokene u `~/.local/share/exa/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Exa, zajedno s ugrađenim alatima. Možete njima upravljati kroz Exa konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Exa sa vašim Sentry nalogom.
