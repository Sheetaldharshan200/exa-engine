---
title: Server
description: Komunicirajte s exa serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `exa serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji exa klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `exa.local` |
...
Postavite `EXA_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `exa`, ili postavite `EXA_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `exa serve` i za `exa web`.
...
Kada pokrenete `exa`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite exa server za programsku interakciju sa exa-om.
:::

Ova arhitektura omogucava exa podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `exa serve` da startate standalone server. Ako je exa TUI vec pokrenut, `exa serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Exa [IDE](/docs/ide) pluginovi.
...
exa server izlaže sljedece API-je.
