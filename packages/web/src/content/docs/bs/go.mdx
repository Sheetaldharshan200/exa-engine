...
Exa Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u Exa-u. Pretplatite se na Exa Go i
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Exa-om.
:::
...
Exa Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

Exa Go radi kao bilo koji drugi provajder u Exa-u.

1. Prijavite se na **<a href={console}>Exa Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Exa Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Exa Go.
:::
...
Exa Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Exa konfiguraciji
koristi format `exa-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`exa-go/kimi-k3` u svojoj konfiguraciji.
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Exa koristite i bilo kojeg drugog provajdera.
