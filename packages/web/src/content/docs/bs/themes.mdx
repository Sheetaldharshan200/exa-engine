...
U Exa mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Exa koristi nasu `exa` temu.
...
Exa dolazi sa vise ugradenih tema.
...
- Zele da Exa odgovara izgledu njihovog terminala
...
Exa podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/exa/themes/*.json` ili `$XDG_CONFIG_HOME/exa/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.exa/themes/*.json`
4. **Trenutni radni direktorij** - `./.exa/themes/*.json`
