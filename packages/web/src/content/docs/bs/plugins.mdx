---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Exa.
---

Dodaci vam omogućavaju da proširite Exa spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Exa.
...
- `.exa/plugins/` - Dodaci na nivou projekta
- `~/.config/exa/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/exa/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/exa/exa.json`)
2. Konfiguracija projekta (`exa.json`)
3. Globalni direktorij dodataka (`~/.config/exa/plugins/`)
4. Direktorij dodataka projekta (`.exa/plugins/`)
...
Exa pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Exa.
...
Spriječite exa da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u exa:
...
Pomoćnik `tool` kreira prilagođeni alat koji exa može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za exa zajedno sa ugrađenim alatima.
