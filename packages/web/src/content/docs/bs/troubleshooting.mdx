...
Da biste otklonili probleme s Exa, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\exa\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `exa --log-level DEBUG`.

---

## Pohrana

Exa pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/exa/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\exa`
...
Exa Desktop pokreće lokalni Exa server (`exa-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Exa` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/exa/exa.jsonc` (ili `~/.config/exa/exa.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/exa/exa.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\exa\exa.jsonc`
...
Exa također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/exa/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\exa\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Exa mogao ponovo izgraditi.

1. Potpuno zatvorite Exa Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/exa`
- **Linux**: obrišite `~/.cache/exa` (ili pokrenite `rm -rf ~/.cache/exa`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\exa`

3. Ponovo pokrenite Exa Desktop.

---

### Rješavanje problema sa vezom na serveru

Exa Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `exa.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `EXA_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `EXA_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Exa Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Exa karakteristikama.

---

### Obavještenja se ne prikazuju

Exa Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Exa u postavkama vašeg OS-a, i
...
1. Zatvorite Exa Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Exa Desktop aplikacije):

- `exa.settings.dat` (URL zadanog servera za desktop)
- `exa.global.dat` i `exa.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `exa upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `exa models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\exa`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Exa dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\exa`

2. Ponovo pokrenite Exa da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Exa da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Exa će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
