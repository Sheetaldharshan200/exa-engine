---
title: Konfiguracija
description: Korištenje Exa JSON konfiguracije.
---

Možete konfigurirati Exa koristeći JSON konfiguracijski fajl.

---

## Format

Exa podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/exa`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/exa/exa.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`EXA_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`exa.json` u projektu) - postavke specifične za projekat
5. **`.exa` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`EXA_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.exa` i `~/.config/exa` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/exa` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Exa konfiguraciju u `~/.config/exa/exa.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/exa/tui.json`.
...
Dodajte `exa.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Exa pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `EXA_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `EXA_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.exa` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.exa`, tako da **može nadjačati** njihove postavke.
...
Koristite `EXA_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `exa.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `exa serve` i `exa web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Exa server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `exa.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Exa konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Exa pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/exa/agents/` ili `.exa/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Exa će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`exa run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/exa/commands/` ili `.exa/commands/`. [Saznajte više ovdje](/docs/commands).
...
Exa će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Exa **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Exa sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.exa/plugins/` ili `~/.config/exa/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Exa da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
