---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Exa.
---

import config from "../../../../config.mjs"
export const console = config.console

Exa koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Exa konfiguraciji.
...
u `~/.local/share/exa/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Exa
config.
...
Exa Zen je lista modela koje je obezbedio Exa tim koji su bili
testirano i potvrđeno da dobro radi sa Exa. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Exa Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite exa i idite na [opencode.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u Exa i potpuno je opcionalan za korištenje.

---

## Exa Go

Exa Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Exa tim i koji su testirani i verificirani da dobro rade s Exa-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Exa Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s Exa:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete exa:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `exa.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Exa.
:::
...
Možete konfigurirati exa za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi exa ispravno radio.
   :::
...
Također možete dodati modele kroz svoju exa konfiguraciju.
...
da pohrani token u exa auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Exa koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Exa
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`exa.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Da biste koristili svoju GitHub Copilot pretplatu s exa:
...
Za korištenje Google Vertex AI s Exa:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće exa.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem exaa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [exa-helicone-session](https://github.com/H2Shami/exa-helicone-session) da automatski prijavite svaki Exa razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Exa razgovor naveden kao zasebna sesija.
...
Možete konfigurirati exa za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati exa za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati exa za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Exa. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/exa) za detalje.
:::
...
Da biste koristili Ollama Cloud s Exa:
...
3. Kopirajte API ključ za korištenje u Exa.
...
6. **Važno**: Prije upotrebe modela oblaka u Exa, morate lokalno povući informacije o modelu:
...
Exa Zen je lista testiranih i verifikovanih modela koju je obezbedio Exa tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Exa Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Exa Zen**.

```txt
   /connect
```

3. Unesite svoj Exa API ključ.
...
Također možete dodati dodatne modele putem vaše exa konfiguracije.
...
5. Također ih možete prilagoditi putem vaše exa konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju exa konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s exa-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `exa.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Exa da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `exa auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Exa konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj exa konfiguraciji.
