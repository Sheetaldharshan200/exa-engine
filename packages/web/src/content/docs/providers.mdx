...
Exa Zen is a list of models provided by the Exa team that have been
tested and verified to work well with Exa. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Exa Zen.
:::

1. Run the `/connect` command in the TUI, select `Exa Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in Exa and is completely optional to use.

---

## Exa Go

Exa Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Exa team that have been
tested and verified to work well with Exa.

1. Run the `/connect` command in the TUI, select `Exa Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `exa.json`:
...
Exa. Anthropic explicitly prohibits this.

Previous versions of Exa came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Exa with zero setup:
...
```json title="exa.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "atomic-chat": {
...
```json title="exa.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
```json title="exa.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "lmstudio": {
...
```json title="exa.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "ollama": {
...
Exa Zen is a list of tested and verified models provided by the Exa team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Exa Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Exa Zen**.
...
4. Create or update your `exa.json` file in your project directory:

   ```json title="exa.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="exa.json" {9,11,17-20}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "myprovider": {
