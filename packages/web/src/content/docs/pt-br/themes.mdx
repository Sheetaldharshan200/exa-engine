...
Com o exa, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o exa usa nosso próprio tema `exa`.
...
O exa vem com vários temas embutidos.
...
- Querem que o exa corresponda à aparência do seu terminal
...
O exa suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/exa/themes/*.json` ou `$XDG_CONFIG_HOME/exa/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.exa/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.exa/themes/*.json`
