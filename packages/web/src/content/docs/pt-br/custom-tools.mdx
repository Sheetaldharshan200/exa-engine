...
description: Crie ferramentas que o LLM pode chamar no exa.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do exa, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.exa/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/exa/tools/`.
