---
title: Plugins
description: Escreva seus próprios plugins para estender o exa.
---

Plugins permitem que você estenda o exa conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do exa.
...
- `.exa/plugins/` - Plugins em nível de projeto
- `~/.config/exa/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/exa/node_modules/`.
...
1. Configuração global (`~/.config/exa/exa.json`)
2. Configuração do projeto (`exa.json`)
3. Diretório de plugins global (`~/.config/exa/plugins/`)
4. Diretório de plugins do projeto (`.exa/plugins/`)
...
O exa executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do exa para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o exa.
...
:::note
Se você estiver usando o aplicativo desktop exa, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o exa de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao exa:
...
O helper `tool` cria uma ferramenta personalizada que o exa pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o exa junto com as ferramentas integradas.
