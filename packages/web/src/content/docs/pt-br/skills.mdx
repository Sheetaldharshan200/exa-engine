...
As habilidades do agente permitem que o exa descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O exa pesquisa nesses locais:

- Configuração do projeto: `.exa/skills/<name>/SKILL.md`
- Configuração global: `~/.config/exa/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o exa sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.exa/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/exa/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.exa/skills/git-release/SKILL.md` assim:
...
O exa lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `exa.json`:
...
**Para agentes embutidos** (em `exa.json`):
