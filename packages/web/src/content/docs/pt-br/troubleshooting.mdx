...
Para depurar problemas com o exa, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\exa\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `exa --log-level DEBUG`.

---

## Armazenamento

O exa armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/exa/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\exa`
...
O exa Desktop executa um servidor exa local (o sidecar `exa-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Exa` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/exa/exa.jsonc` (ou `~/.config/exa/exa.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/exa/exa.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\exa\exa.jsonc`
...
O exa também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/exa/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\exa\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o exa possa reconstruí-lo.

1. Saia completamente do exa Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/exa`
- **Linux**: exclua `~/.cache/exa` (ou execute `rm -rf ~/.cache/exa`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\exa`

3. Reinicie o exa Desktop.

---

### Corrigindo problemas de conexão com o servidor

O exa Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `exa.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `EXA_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `EXA_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o exa Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do exa.

---

### Notificações não aparecendo

O exa Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o exa nas configurações do seu sistema operacional, e
...
1. Saia do exa Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo exa Desktop):

- `exa.settings.dat` (URL do servidor padrão do desktop)
- `exa.global.dat` e `exa.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o exa:
...
3. Certifique-se de que você tem a versão mais recente com `exa upgrade`
...
Para descobrir quais modelos você tem acesso, execute `exa models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\exa`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O exa instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\exa`

2. Reinicie o exa para reinstalar os pacotes de provedores mais recentes

Isso forçará o exa a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O exa detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
