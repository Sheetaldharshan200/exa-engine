---
title: Servidor
description: Interaja com o servidor exa via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `exa serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente exa pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `exa.local` |
...
Defina `EXA_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `exa`, ou defina `EXA_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `exa serve` quanto ao `exa web`.
...
Quando você executa `exa`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor exa para interagir com o exa programaticamente.
:::

Essa arquitetura permite que o exa suporte múltiplos clientes e permite que você interaja com o exa programaticamente.

Você pode executar `exa serve` para iniciar um servidor autônomo. Se você tiver o TUI do exa em execução, `exa serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do exa [IDE](/docs/ide).
...
O servidor exa expõe as seguintes APIs.
