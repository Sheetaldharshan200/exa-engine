---
title: الخادم
description: تفاعل مع خادم exa عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `exa serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل exa استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `exa.local` |
...
عيّن `EXA_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `exa`، أو عيّن `EXA_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `exa serve` و `exa web`.
...
عند تشغيل `exa` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم exa للتفاعل مع exa برمجيا.
:::

تتيح هذه البنية لـ exa دعم عدة عملاء وتمكّنك من التفاعل مع exa برمجيا.

يمكنك تشغيل `exa serve` لبدء خادم مستقل. إذا كانت واجهة exa في terminal (TUI)
قيد التشغيل، فسيبدأ `exa serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Exa لـ [IDE](/docs/ide).
...
يعرض خادم exa واجهات API التالية.
