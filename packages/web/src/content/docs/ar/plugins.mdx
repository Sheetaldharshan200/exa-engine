---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Exa.
---

تتيح لك الإضافات توسيع Exa عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Exa.
...
- `.exa/plugins/` - إضافات على مستوى المشروع
- `~/.config/exa/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/exa/node_modules/`.
...
1. إعدادات عامة (`~/.config/exa/exa.json`)
2. إعدادات المشروع (`exa.json`)
3. دليل الإضافات العام (`~/.config/exa/plugins/`)
4. دليل إضافات المشروع (`.exa/plugins/`)
...
يشغّل Exa الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ exa للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع exa.
...
:::note
إذا كنت تستخدم تطبيق Exa لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى exa:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ exa استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ exa إلى جانب الأدوات المضمنة.
