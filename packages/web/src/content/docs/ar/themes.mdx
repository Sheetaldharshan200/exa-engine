...
مع Exa يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Exa سمتنا `exa`.
...
- يريدون أن يطابق Exa مظهر terminal لديهم
...
يدعم Exa نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/exa/themes/*.json` أو `$XDG_CONFIG_HOME/exa/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.exa/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.exa/themes/*.json`
