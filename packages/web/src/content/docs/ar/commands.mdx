...
أنشئ الملف `.exa/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Exa أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Exa:
...
- على مستوى النظام: `~/.config/exa/commands/`
- لكل مشروع: `.exa/commands/`
...
يتضمن exa عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
