---
title: المزوّدون
description: استخدام أي موفّر LLM في Exa.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Exa ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Exa.
...
في `~/.local/share/exa/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Exa.
...
Exa Zen هي قائمة نماذج يوفّرها فريق Exa وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Exa. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Exa Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر exa، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/auth).
...
يعمل مثل أي مزوّد آخر في Exa واستخدامه اختياري بالكامل.

---

## Exa Go

Exa Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Exa، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Exa.

1. شغّل الأمر `/connect` في TUI، واختر `Exa Go`، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع Exa:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل exa:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `exa.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Exa غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين exa لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل exa بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات exa.
...
تخزين الرمز في مساحة تخزين مصادقة exa.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Exa نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Exa
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`exa.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع exa:
...
لاستخدام Google Vertex AI مع Exa:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر exa، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [exa-helicone-session](https://github.com/H2Shami/exa-helicone-session) لتسجيل كل محادثة Exa تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Exa مدرجة كجلسة مستقلة.
...
يمكنك ضبط exa لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط exa لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط exa لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Exa. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/exa) للتفاصيل.
:::
...
3. انسخ مفتاح API لاستخدامه في Exa.
...
6. **مهم**: قبل استخدام نماذج السحابة في Exa، يجب سحب معلومات النموذج محليا:
...
Exa Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Exa. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Exa Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Exa Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Exa.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات exa.
...
5. يمكنك أيضا تخصيصها عبر إعدادات exa. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات exa. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع exa. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `exa.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Exa بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `exa auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات exa وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات exa.
