...
Exa Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Exa. Du abonnierst Exa Go und
...
Exa Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Exa Go funktioniert wie jeder andere Provider in Exa.

1. Du meldest dich bei **<a href={console}>Exa Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Exa Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Exa Go abonnieren.
:::
...
Exa Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Exa Config verwendet das Format `exa-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `exa-go/kimi-k3` in deiner Config verwenden.
...
Wir haben Exa Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Exa zu nutzen.
