...
Wenn Exa Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\exa\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `exa --log-level DEBUG`.

---

## Speicher

exa speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/exa/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\exa`
...
Exa Desktop startet im Hintergrund einen lokalen Exa-Server (`exa-cli`-Sidecar).
...
- Nur macOS: `Exa`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/exa/exa.jsonc` (oder `~/.config/exa/exa.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/exa/exa.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\exa\exa.jsonc`
...
Exa kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/exa/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\exa\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Exa ihn neu aufbauen kann.

1. Beende Exa Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/exa`
- **Linux**: loesche `~/.cache/exa` (oder fuehre aus: `rm -rf ~/.cache/exa`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\exa`

3. Starte Exa Desktop neu.

---

### Server-Verbindungsprobleme beheben

Exa Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `exa.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `EXA_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `EXA_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Exa Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Exa-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Exa Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Exa in den OS-Einstellungen aktiviert sind, und
...
1. Beende Exa Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Exa Desktop):

- `exa.settings.dat` (desktop default server URL)
- `exa.global.dat` und `exa.workspace.*.dat` (UI state like recent servers/projects)
...
3. Stelle sicher, dass du die neueste Version hast: `exa upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `exa models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\exa`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. exa installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\exa`

2. Starte exa neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt exa, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
exa erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
