---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Exa zu erweitern.
---

Mit Plugins können Sie Exa erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Exa zu ändern.
...
- `.exa/plugins/` – Plugins auf Projektebene
- `~/.config/exa/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/exa/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/exa/exa.json`)
2. Projektkonfiguration (`exa.json`)
3. Globales Plugin-Verzeichnis (`~/.config/exa/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.exa/plugins/`)
...
Exa führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Exa-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Exa erweitern können.
...
:::note
Wenn Sie die Exa-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Exa `.env`-Dateien liest:
...
Plugins können Exa auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Exa aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Exa zur Verfügung.
