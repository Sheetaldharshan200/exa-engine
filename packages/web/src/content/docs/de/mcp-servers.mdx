...
Sie können externe Tools zu Exa hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Exa unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Exa Config](https://opencode.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/exa`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Exa übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Exa Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Exa beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `exa mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Exa die Token sicher in `~/.local/share/exa/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Exa verfügbar. Sie können sie also wie jedes andere Tool über die Exa-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Exa mit Ihrem Sentry-Konto zu verbinden.
