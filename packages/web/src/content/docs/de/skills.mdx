...
Agent Skills erlauben Exa, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Bei projektlokalen Pfaden laeuft Exa vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.exa/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/exa/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.exa/skills/git-release/SKILL.md` zum Beispiel so:
...
Exa listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `exa.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `exa.json`):
