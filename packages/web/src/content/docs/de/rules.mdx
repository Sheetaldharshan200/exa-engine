---
title: Regeln
description: Lege eigene Anweisungen fuer exa fest.
---

Du kannst exa ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in exa den Befehl `/init` aus.
...
So kann exa sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht exa sie zu erweitern.
...
exa kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/exa/AGENTS.md` speichern.
Diese gelten in allen exa-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Exa dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/exa/AGENTS.md` existiert)
...
Beim Start sucht exa in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/exa/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `exa.json` oder in `~/.config/exa/exa.json` angeben.
...
exa parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit exa.json

Empfohlen ist das `instructions`-Feld in der `exa.json`:
...
Du kannst exa explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `exa.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
