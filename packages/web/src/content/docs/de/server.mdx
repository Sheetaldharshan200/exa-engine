---
title: Server
description: Nutze den exa-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `exa serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein exa-Client nutzen kann.
...
Setze `EXA_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `exa`, kann aber mit `EXA_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `exa serve` und `exa web`.
...
Wenn du `exa` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den exa-Server, um exa programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `exa serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `exa serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Exa-[IDE](/docs/ide)-Plugins.
...
Der exa-Server stellt folgende APIs bereit.
