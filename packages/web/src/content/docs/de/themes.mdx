...
Mit Exa kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Exa das `exa`-Theme.
...
Exa liefert mehrere eingebaute Themes mit.
...
- Exa optisch ans Terminal anpassen wollen
...
Exa unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/exa/themes/*.json` oder `$XDG_CONFIG_HOME/exa/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.exa/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.exa/themes/*.json`
