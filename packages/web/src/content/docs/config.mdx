...
```json title="Remote config from .well-known/exa"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
TUI settings live in `tui.json` next to your config; the keys are documented in [TUI](/docs/tui) and [Keybinds](/docs/keybinds).
...
You can also define agents using markdown files in `~/.config/exa/agents/` or `.exa/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Exa will fall back to `"build"` with a warning.
...
You can also define commands using markdown files in `~/.config/exa/commands/` or `.exa/commands/`. [Learn more here](/docs/commands).
...
[Plugins](/docs/plugins) extend Exa with custom tools, hooks, and integrations.
