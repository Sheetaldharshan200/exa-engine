...
import { Event } from "@exa/schema/event"
import { SessionStatusEvent } from "@exa/schema/session-status-event"
import { SessionV1 } from "@exa/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/Exa/TimelineStability"
...
export const model = { providerID: "exa", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("exa.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["exa"],
    default: { providerID: "exa", modelID: "claude-opus-4-6" },
  }
}
