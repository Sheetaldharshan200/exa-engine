...
import type { retry } from "@exa/core/util/retry"
import type { ExaEvent, SessionApi } from "@exa-ai/client/promise"
import type { Message, ExaClient, Part, Session } from "@exa/sdk/v2/client"
...
describe("server session", () => {
  test("projects V2 session events into current and legacy message state", () => {
    ...
    const apply = (input: object) => ctx.store.applyV2(input as ExaEvent)
  ...
  test("extends a current page to include the user for split assistant turns", async () => {
    ...
    const store = createServerSession({} as ExaClient, {} as SessionApi, messageApi)
    store.remember(session("root"))
