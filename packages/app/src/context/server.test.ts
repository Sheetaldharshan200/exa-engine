  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://exa.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://exa.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://exa.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://exa.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://exa.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://exa.example.com"),
      ),
    ).toEqual({
