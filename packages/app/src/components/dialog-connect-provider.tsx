import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@exa-ai/client/promise"
...
import { List, type ListRef } from "@exa/ui/list"
import { ProviderIcon } from "@exa/ui/provider-icon"
import { Spinner } from "@exa/ui/spinner"
import { Tag } from "@exa/ui/tag"
import { TextField } from "@exa/ui/text-field"
import { ButtonV2 } from "@exa/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@exa/ui/v2/dialog-v2"
import { TextInputV2 } from "@exa/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "exa-go") return language.t("dialog.provider.exaGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "exa"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.exa.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["exa", "exa-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "exa" || provider.id === "exa-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "exa"
                                ? "dialog.provider.exa.tagline"
                                : "dialog.provider.exaGo.tagline",
                            )}
                          </span>
        ...
        <Switch>
          <Match when={provider().id === "exa"}>
            <div class="flex flex-col gap-4">
              ...
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.exaZen.visit.prefix")}
                <ExternalLink href="https://exasol.com/exa/zen" tabIndex={-1}>
                  {language.t("provider.connect.exaZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.exaZen.visit.suffix")}
              </div>
