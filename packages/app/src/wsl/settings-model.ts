...
export function wslExaAction(check?: WslExaCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installExa"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateExa"
}
...
export function addServerViewModel(input: {
  ...
  const exaCheck = selectedDistro ? (state?.exaChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.exaChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateExa" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.exaMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installExa" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  ...
  const installingExa =
    input.state?.job?.kind === "install-exa" && input.state.job.distro === input.selectedDistro
  if (!ready || probingExa) {
    return {
      variant: "contrast",
  ...
  if (!addServerExaReady(input.exaCheck)) {
    const update = !!input.exaCheck?.resolvedPath && input.exaCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingExa
        ? { key: "wsl.onboarding.updatingExa" }
        : update
          ? { key: "wsl.onboarding.updateExa" }
          : { key: "wsl.onboarding.installExa" },
      disabled: !!input.state?.job || input.adding,
      action: "install-exa",
      loading: installingExa,
...
function addServerExaReady(check: WslExaCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.exaChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.exaChecks[item.name]) return [`exa:${item.name}`]
    return []
  })
