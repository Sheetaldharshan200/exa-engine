...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Exa needs attention", () => {
    expect(wslExaAction(undefined)).toBeUndefined()
    expect(
      wslExaAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("wsl.onboarding.installExa")
    expect(
      wslExaAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/exa",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("wsl.onboarding.updateExa")
    expect(
      wslExaAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/exa",
  ...
  test("does not report ready when Exa is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        exaChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.exa/bin/exa",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "exa is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installExa" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-exa")
  })
