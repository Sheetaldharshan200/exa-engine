...
import { wslExaAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.exaChecks[item.config.distro]
          const exaAction = () => wslExaAction(check())
          const busy = () => wsl.data?.job?.kind === "install-exa" && wsl.data.job.distro === item.config.distro
          return (
                ...
                <Show when={exaAction()}>
                  {(label) => (
                    <ButtonV2
                      ...
                      onClick={() => api && request.mutate(() => api.installExa(item.config.distro))}
                    >
