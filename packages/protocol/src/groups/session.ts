...
import { PromptInput } from "@exa/schema/prompt-input"
import { Session } from "@exa/schema/session"
import { Project } from "@exa/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@exa/schema/schema"
...
import { SessionEvent } from "@exa/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Exa process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Exa process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
