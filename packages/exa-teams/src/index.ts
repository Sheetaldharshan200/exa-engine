...
import { ExaClient } from "@exa/sdk/v2"
...
const plugin: Plugin = async (input) => {
  ...
  // to the v2 ExaClient. The plugin framework provides a v1 client which stores its
  // HeyAPI transport as `_client` (underscore). The v2 constructor accepts it as `client`.
  type V2Transport = ConstructorParameters<typeof ExaClient>[0] extends { client?: infer C } ? C : never
  const pluginTransport = (input.client as unknown as { _client: V2Transport })._client
  const rawClient = new ExaClient({ client: pluginTransport })
  initLog(rawClient)
