import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@exa/sdk/v2"
import { SessionTurn } from "@exa/session-ui/session-turn"
import { SessionReview } from "@exa/session-ui/session-review"
import { DataProvider } from "@exa/session-ui/context"
import { FileComponentProvider } from "@exa/ui/context/file"
import { WorkerPoolProvider } from "@exa/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@exa/ui/logo"
import { IconButton } from "@exa/ui/icon-button"
import { ProviderIcon } from "@exa/ui/provider-icon"
import { iife } from "@exa/core/util/iife"
import { Binary } from "@exa/core/util/binary"
import { NamedError } from "@exa/core/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import NotFound from "../[...404]"
import { Tabs } from "@exa/ui/tabs"
import { MessageNav } from "@exa/session-ui/message-nav"
import { FileSSR } from "@exa/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@exa/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/exa-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | Exa</Title>
              </Show>
              <Meta name="description" content="exa - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/anomalyco/exa"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
