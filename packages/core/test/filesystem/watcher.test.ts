...
import { Watcher } from "@exa/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    EXA_EXPERIMENTAL_FILEWATCHER: "true",
    EXA_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@exa.test`.cwd(tmp.path).quiet()
