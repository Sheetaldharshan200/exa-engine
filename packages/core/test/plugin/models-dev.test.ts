...
import { ModelsDevPlugin } from "@exa/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.EXA_MODELS_PATH,
          disabled: Flag.EXA_DISABLE_MODELS_FETCH,
        }
        Flag.EXA_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.EXA_MODELS_PATH = previous.path
          Flag.EXA_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
