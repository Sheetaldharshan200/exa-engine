...
import { Config } from "@exa/core/config"
import { ConfigProvider } from "@exa/core/config/provider"
import { AppNodeBuilder } from "@exa/core/effect/app-node-builder"
import { LayerNode } from "@exa/core/effect/layer-node"
import { ConfigMigrateV1 } from "@exa/core/v1/config/migrate"
...
import { Policy } from "@exa/core/policy"
import { Project } from "@exa/core/project"
import { AbsolutePath } from "@exa/core/schema"
...
describe("Config", () => {
  ...
  it.live("loads exa JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "exa.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "exa.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "exa.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "exa.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "exa.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "exa.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .exa configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".exa"), { recursive: true })
            await fs.mkdir(path.join(directory, ".exa"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "exa.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "exa.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "exa.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "exa.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "exa.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".exa", "exa.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".exa", "exa.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".exa")),
              AbsolutePath.make(path.join(directory, ".exa")),
            ])
