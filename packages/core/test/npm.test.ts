...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@exa/acme")).toBe("@exa/acme")
    expect(Npm.sanitize("@exa/acme@1.0.0")).toBe("@exa/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/exa/acme.git"
    const expected = win ? "acme@git+https_//github.com/exa/acme.git" : spec
