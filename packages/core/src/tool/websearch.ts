...
import { ToolFailure } from "@exa/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@exa/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.EXA_WEBSEARCH_PROVIDER === "exa" || process.env.EXA_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.EXA_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("EXA_EXPERIMENTAL") || truthy("EXA_ENABLE_EXA") || truthy("EXA_EXPERIMENTAL_EXA"),
    enableParallel: truthy("EXA_ENABLE_PARALLEL") || truthy("EXA_EXPERIMENTAL_PARALLEL"),
