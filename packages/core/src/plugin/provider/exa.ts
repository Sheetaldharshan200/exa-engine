...
import type { IntegrationOAuthMethodRegistration } from "@exa/plugin/v2/effect/integration"
import { define } from "@exa/plugin/v2/effect/plugin"
import type { CredentialValue } from "@exa/sdk/v2/types"
...
export const ExaPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "exa",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("ExaPlugin.load")(function* () {
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Exa provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("exa", (integration) => {
        integration.name = "Exa"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "exa", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("exa")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("exa")
      ...
      const item = catalog.provider.get(ProviderV2.ID.exa)
      if (!item) return
      const hasKey = Boolean(process.env.EXA_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("exa")),
