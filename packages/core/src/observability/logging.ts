...
export function fileLogger(file = path.join(Global.Path.log, "exa.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.EXA_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.EXA_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
