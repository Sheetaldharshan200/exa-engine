...
import { createOpencode, type ToolPart } from "@exa/sdk"
...
console.log("🚀 Starting exa server...")
const exa = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await exa.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new exa session...")
    const { client, server } = exa
    ...
    console.log("✅ Created exa session:", createResult.data.id)
  ...
  console.log("📝 Sending to exa:", message.text)
