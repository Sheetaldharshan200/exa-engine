...
import { createOpencode } from "@exa/sdk"
...
`)
  ...
  const exa = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await exa.client.session.create()
    const result = await exa.client.session
      .prompt({
