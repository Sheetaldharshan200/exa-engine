...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "exa/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".exa/glossary/zh-cn.md"
  if (locale === "zht") return ".exa/glossary/zh-tw.md"
  return `.exa/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Exa did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Exa returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Exa session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Exa runs for 'all' (default: 4)
      --model <provider/id>  Exa model (default: exa/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Exa
...
`)
  ...
  if (failed.length) console.error(`\nExa failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.EXA_DISABLE_PROJECT_CONFIG = "1"
  env.EXA_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["exa", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.EXA_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["exa", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.EXA_CONFIG
  delete env.EXA_CONFIG_DIR
  delete env.EXA_CONFIG_CONTENT
  delete env.EXA_PERMISSION
  delete env.EXA_AUTO_SHARE
  return env
}
