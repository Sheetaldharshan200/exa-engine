...
const model = "exa/deepseek-v4-flash"
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with exa...`)
  ...
  try {
    await $`exa run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  exa failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with exa")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with exa...")
