...
Both `action` and `resource` use exa's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.exa` files is intentionally deferred until `.exa` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether exa permits an operation through its managed execution paths.
