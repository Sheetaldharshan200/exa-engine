...
The exa server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@exa/llm` tool loop after replacing its
...
- plugins get global "exa" instance like in that post i showed
- exa instance has stuff like `exa.session.prompt()` or
  `exa.tool.register({...})`
...
- decide whether to preserve deprecated `@exa/llm` orchestration exports
