...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/exa`.
- Do not replace private `packages/exa` imports with `@exa/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@exa/tui`.
...
```text
packages/exa/src/cli/cmd/tui
```

Its private dependency on `packages/exa` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/exa/src/*`.
...
Status: Completed. The private `@exa/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@exa/tui`.
...
primitives now live in `@exa/tui`. The legacy host consumes explicit
...
`@exa/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/exa` initially.
...
canonical in `@exa/tui`. Configured references resolve through the new
...
`@exa/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/exa`.
...
- The package has no imports from `packages/exa`, `packages/cli`, or
  `@exa/core`.
...
public `@exa/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/exa`.
- Keep the legacy embedded worker and server startup in `packages/exa`.
...
- Remove obsolete `@tui/*` path mappings from `packages/exa`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@exa/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@exa/core`, or either
  executable package.
...
rg '@exa/core|packages/exa|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/exa/src packages/exa/test
```
...
- Build the current-platform `packages/exa` binary.
