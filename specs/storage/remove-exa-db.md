# Remove `packages/exa/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/exa/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the exa legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/exa/src/storage/db.ts` are concentrated in 22 source files:

- `packages/exa/src/account/repo.ts`
- `packages/exa/src/cli/cmd/db.ts`
- `packages/exa/src/cli/cmd/import.ts`
- `packages/exa/src/cli/cmd/stats.ts`
- `packages/exa/src/control-plane/workspace.ts`
- `packages/exa/src/index.ts`
- `packages/exa/src/node.ts`
- `packages/exa/src/permission/index.ts`
- `packages/exa/src/project/project.ts`
- `packages/exa/src/server/projectors.ts`
- `packages/exa/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/exa/src/server/shared/fence.ts`
- `packages/exa/src/session/message-v2.ts`
- `packages/exa/src/session/projectors.ts`
- `packages/exa/src/session/prompt.ts`
- `packages/exa/src/session/session.ts`
- `packages/exa/src/session/todo.ts`
- `packages/exa/src/share/share-next.ts`
- `packages/exa/src/storage/db.ts`
- `packages/exa/src/sync/index.ts`
- `packages/exa/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy exa database wrapper; `packages/exa/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the exa wrapper.
...
Status: Completed. `SyncEvent` and the exa projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy exa database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy exa database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy exa database wrapper.
...
All migration groups are complete or superseded. `packages/exa/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No exa data-migration group remains.

The previous exa `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate exa data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/exa`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/exa/src`
- `bun typecheck` from `packages/exa`
- Relevant package tests from `packages/exa`, not the repo root
